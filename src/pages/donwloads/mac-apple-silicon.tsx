...
const MacAppleSiliconDownload: React.FunctionComponent = () => {
  function downloadInstaller(e){
    console.log('Download Button was clicked');
    const urlSearchParams = new URLSearchParams(window.location.search);
    const ver = urlSearchParams.get('ver');
    if (ver) {
        var downloadUrl = `https://download.gettrici.com/darwin-x64/Trici-${ver}-arm64.dmg`;
        window.open(downloadUrl, '_blank');
    } else {
        window.open('https://download.gettrici.com/darwin-x64/Trici-2.5.0-arm64.dmg');
    }
  }
  return (
    <Layout>
...
export default MacAppleSiliconDownload
